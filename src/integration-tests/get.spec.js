...
describe("Retrieve data from /test/test.json", () => {
  ...
  describe("error cases", () => {
    test("should return 403 if bad password", () =>
      request(app).get("/test/test.json").auth(user, "bad password").expect(403));
    ...
    test("should respond 404 for not existing file", () =>
      request(app).get("/test/notexisting.json").auth(user, password).expect(404));

    test("should respond 404 for empty key", () => request(app).get("/test/.json").auth(user, password).expect(404));

    test("should respond 404 for empty app", () => request(app).get("//a.json").auth(user, password).expect(404));
